...
private:
	...
	Predicate parsePredicate(int type) {
	...
	void parsePredicateList(vector<Predicate>& predList, int type) {
	...
	void parseRule() {
		Predicate headPredicate = parsePredicate(RULE_OR_QUERY);
		vector<Predicate> predList;
		
		match(COLON_DASH);
		predList.push_back(parsePredicate(RULE_OR_QUERY));
		parsePredicateList(predList, RULE_OR_QUERY);
		match(PERIOD);
	...
	void parseQuery() {
		program->addQuery(parsePredicate(RULE_OR_QUERY));
		match(Q_MARK);
	}
	...
	Token parseParameter(int type) {				
		switch(tokens->front().type()) {
			case ID: 
				if (type == SCHEME || type == RULE_OR_QUERY) 
					return match(ID);
			case STRING: 
				if (type == FACT || type == RULE_OR_QUERY) 
					return match(STRING);
			default: throw tokens->front();
		}
	}
	
	void parseParameterList(vector<Token>& parameterList, int type) {
		if (tokens->front().type() == COMMA) {
			match(COMMA);
			parameterList.push_back(parseParameter(type));
			parseParameterList(parameterList, type);
